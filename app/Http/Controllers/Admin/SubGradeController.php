...
class SubGradeController extends Controller
{
    ...
    {
        $sub_grades = SubGrade::get();
        if (request()->ajax()) {
            return datatables()->of($sub_grades)
                ->addColumn('action', function ($data) {
                    if (auth()->user()->hasPermission('update_sub_grades')) {
                        $button = '<a type="button" title="Edit" name="edit" href="sub_grades/' . $data->id . '/edit" class="edit btn btn-sm btn-icon"><i class="fa fa-edit"></i></a>';
                    } else {
                        $button = '<a type="button" title="Edit" name="edit" id="' . $data->id . '" class="edit btn btn-sm btn-icon disabled"><i class="fa fa-edit"></i></a>';
                    }
                    $button .= '&nbsp;&nbsp;';
                    if (auth()->user()->hasPermission('delete_sub_grades')) {
        ...
        return view('admin.sub_grades.index');
    }

    public function create()
    {
        return view('admin.sub_grades.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.sub_grades.index');
    }

    public function edit(Facility $facility)
    {
        return view('admin.sub_grades.edit')->with('facility', $facility);
    }

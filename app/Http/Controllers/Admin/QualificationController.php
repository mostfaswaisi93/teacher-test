...
class QualificationController extends Controller
{
        ...
        if (request()->ajax()) {
            return datatables()->of($qualifications)
                ->addColumn('action', function ($data) {
                    if (auth()->user()->hasPermission('update_qualifications')) {
                        $button = '<a type="button" title="Edit" name="edit" href="qualifications/' . $data->id . '/edit" class="edit btn btn-sm btn-icon"><i class="fa fa-edit"></i></a>';
                    } else {
                        $button = '<a type="button" title="Edit" name="edit" id="' . $data->id . '" class="edit btn btn-sm btn-icon disabled"><i class="fa fa-edit"></i></a>';
                    }
                    $button .= '&nbsp;&nbsp;';
                    if (auth()->user()->hasPermission('delete_qualifications')) {
    ...
    {
        return view('admin.qualifications.edit')->with('facility', $facility);
    }

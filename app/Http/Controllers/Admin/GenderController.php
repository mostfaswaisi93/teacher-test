...
class GenderController extends Controller
{
        ...
        if (request()->ajax()) {
            return datatables()->of($genders)
                ->addColumn('action', function ($data) {
                    if (auth()->user()->hasPermission('update_genders')) {
                        $button = '<a type="button" title="Edit" name="edit" href="genders/' . $data->id . '/edit" class="edit btn btn-sm btn-icon"><i class="fa fa-edit"></i></a>';
                    } else {
                        $button = '<a type="button" title="Edit" name="edit" id="' . $data->id . '" class="edit btn btn-sm btn-icon disabled"><i class="fa fa-edit"></i></a>';
                    }
                    $button .= '&nbsp;&nbsp;';
                    if (auth()->user()->hasPermission('delete_genders')) {
    ...
    {
        return view('admin.genders.edit')->with('facility', $facility);
    }
